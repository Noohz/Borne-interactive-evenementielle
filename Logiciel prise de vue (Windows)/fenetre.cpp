...
CFenetre::CFenetre(QWidget *p_parent) : QWidget(p_parent), ui(new Ui::CFenetre)
{
    ...
    camera = new CCamera(QCameraInfo::defaultCamera(), ui->viewfinder, this); //Objet caméra
...
{
    photo = p_photo;
    ui->photoLabel->setPixmap(QPixmap::fromImage(photo.scaled(ui->viewfinder->size(), Qt::KeepAspectRatio))); //On applique la photo au QLabel
